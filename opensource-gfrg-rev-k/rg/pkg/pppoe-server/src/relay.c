...
/* Global static data - to reduce stack usage. The functions that use this
 * should be non-reentrant */
static PPPoETag wrk_tag;
...
static int os_skb_padto(struct sk_buff **buf, int new_size)
{
    struct sk_buff *skb = *buf;
#ifdef CONFIG_RG_OS_LINUX_26
    int ret;

    if ((ret = skb_padto(skb, new_size)))
	return ret;
#else
    if (!(skb = skb_padto(skb, new_size)))
	return -1;
...
addTag(struct sk_buff **m, PPPoETag const *tag)
{
    ...
    new_size = len + offset;
    if ((ret = os_skb_padto(&skb, new_size)))
	return ret;
		...
		struct sk_buff *m)
{
    ...
    loc = findTag(packet, TAG_RELAY_SESSION_ID, &wrk_tag);
    if (!loc) {
	u32 magic = JUNGO_RELAY_MAGIC;

	wrk_tag.type = htons(TAG_RELAY_SESSION_ID);
	wrk_tag.length = htons(MY_RELAY_TAG_LEN);
	memcpy(wrk_tag.payload, &ifIndex, sizeof(ifIndex));
	memcpy(wrk_tag.payload+sizeof(ifIndex), packet->ethHdr.h_source,
	    ETH_ALEN);
	memcpy(wrk_tag.payload+sizeof(ifIndex) + ETH_ALEN, &magic, sizeof(magic));
		...
		struct sk_buff *m)
{
    ...
    if (ntohs(wrk_tag.length) != MY_RELAY_TAG_LEN) {
	printk(KERN_ERR
    ...
    memcpy(&magic, wrk_tag.payload+MY_RELAY_TAG_LEN-sizeof(magic),
	sizeof(magic));
    if (magic!=JUNGO_RELAY_MAGIC)
	return -1;
    
    /* Extract interface index */
    memcpy(&ifIndex, wrk_tag.payload, sizeof(ifIndex));
    ...
    memcpy(packet->ethHdr.h_dest, wrk_tag.payload + sizeof(ifIndex), ETH_ALEN);
		...
		struct sk_buff *m)
{
	...
	if (!ses) {
		...
		/* Can't allocate session -- send error PADS to client and
		   PADT to server */
		PPPoETag *hu;
		if (findTag(packet, TAG_HOST_UNIQ, &wrk_tag)) {
		    hu = &wrk_tag;
		} else {
		    hu = NULL;
		}
