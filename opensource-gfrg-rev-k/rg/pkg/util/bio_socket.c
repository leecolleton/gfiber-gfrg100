...
int bio_accept(int server_fd)
{
    ...
    /* In some implementation of accept(), it can't receive NULL as its second
     * parameter, this is why we use the dummy buffer even though we don't
     * care for the value it receives.
     */
    while ((fd = accept(server_fd, (struct sockaddr *)&dummy_addr,
	(socklen_t *)&dummy_addr_len)) < 0)
    {
