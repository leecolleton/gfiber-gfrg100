...
int input_cmdline = 0;  /* 1: read partition table spec from command line */
char *cmdline_desc = NULL;
...
read_stdin(unsigned char **fields, unsigned char *line, int fieldssize, int linesize) {
    ...
    {
	static char *cmdline_cur;
	char *eol;
	int l;

	if (!cmdline_cur)
	    cmdline_cur = cmdline_desc;
	if (!(eol = strchr(cmdline_cur, '!')))
	{
	    eof = 1;
	    return RD_EOF;
	}
	...
	if (l >= linesize)
	    l = linesize - 1;
	lp = strncpy(line+2, cmdline_cur, l);
	cmdline_cur += l + 1;
	lp[l] = '\n';
	lp[l+1] = 0;
    }
...
static char short_opts[] = "cdfgilnqsu:vx?1A::C:DH:I:LN:O:RS:TU::Va:";
