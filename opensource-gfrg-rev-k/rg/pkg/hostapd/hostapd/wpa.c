		 ...
		 u8 *data, size_t data_len)
{
	...
	if (!(key_info & WPA_KEY_INFO_REQUEST) &&
		...
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_WPA,
			       ...
			       "counter", msgtxt);
                wl_alerts_send(hapd->conf->ssid, sta->addr, 0);
		if (HOSTAPD_DEBUG_COND(HOSTAPD_DEBUG_MINIMAL)) {
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	if (!ok) {
		hostapd_logger(sm->hapd, sm->sta->addr, HOSTAPD_MODULE_WPA,
			       ...
			       "of 4-Way Handshake");
                wl_alerts_send(sm->hapd->conf->ssid, sm->sta->addr, 0);
		return;
	}
...
SM_STATE(WPA_PTK, PTKINITDONE)
{
	...
	hostapd_logger(sm->hapd, sm->sta->addr, HOSTAPD_MODULE_WPA,
        ...
        wl_alerts_send(sm->hapd->conf->ssid, sm->sta->addr, 1); 
