    ...
    char *name)
{
    ...
    return (*poolPtrPtr = initFunctionPtr(count,size,name)) ? IX_SUCCESS : IX_FAIL;
#else
...
{
#ifdef CONFIG_RG_OS_LINUX_26
    return (*poolPtrPtr = initNoAllocFunctionPtr(poolBufPtr,poolDataPtr,count,size,name)) ?
	IX_SUCCESS : IX_FAIL;
#else
...
{
#ifdef CONFIG_RG_OS_LINUX_26
    return (*newBufPtrPtr = getFunctionPtr(poolPtr)) ? IX_SUCCESS : IX_FAIL;
#else
