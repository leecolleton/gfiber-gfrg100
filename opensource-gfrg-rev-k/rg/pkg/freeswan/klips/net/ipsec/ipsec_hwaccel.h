...
#else /* new csr */
typedef IX_MBUF_POOL *(*HwAccelOsBuffPoolInit) (
    UINT32 count,
    UINT32 size,
    const char *name);

typedef IX_MBUF_POOL *(*HwAccelOsBuffPoolInitNoAlloc) (
    void *poolBufPtr, 
    void *poolDataPtr, 
    UINT32 count, 
    UINT32 size, 
...
typedef IX_MBUF *(*HwAccelOsBuffPoolUnchainedBufGet) (
    IX_MBUF_POOL *poolPtr);

typedef IX_MBUF * (*HwAccelOsBuffPoolBufFree) (IX_MBUF *bufPtr);

typedef UINT32 (*HwAccelOsBuffPoolBufAreaSizeAligned)(UINT32 count);

typedef void * (*HwAccelOsServCacheDmaAlloc) (UINT32 size);

typedef void (*HwAccelOsServCacheDmaFree) (void *ptr);
#endif
