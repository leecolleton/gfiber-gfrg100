...
time_t time_start;
static int _console_printf(const char *format, ...)
{
    va_list args;
    int rv;
    FILE *fp;
    ...
    fprintf(fp, "%d: ", (int)(time(NULL) - time_start));
...
#undef DBG_log
#define DBG(a, b) b
#define DBG_log(a...) _console_printf(a)
#define log(a...) _console_printf(a)
#define loglog(b, a...) _console_printf(a)
#endif
...
dpd_outI(struct state *p1st, struct state *st, bool eroute_care
	 ,time_t delay, time_t timeout, time_t idle_timeout)
{
    time_t tm;
    time_t last;
    u_int32_t seqno;
    bool   eroute_idle;
    time_t nextdelay, _delay = delay;
    ...
    /* We are not waiting for an R_U_THERE ACK, so use the idle timeout */
    if (!p1st->st_dpd_expectseqno)
	_delay = idle_timeout;
    /*
	...
	eroute_idle = was_eroute_idle(st, idle_timeout, &last_idle);
	if(!eroute_idle) {
	    DBG(DBG_DPD, DBG_log("dpd out event not sent, phase 2 active (idle=%d)", last_idle));
	    
	    /* update phase 2 time stamp only */
	    st->st_last_dpd = tm - last_idle;
	    nextdelay = idle_timeout - last_idle;
	 ...
	 * routine.
	 * We are using delay here because an R_U_THERE is about to be sent.
	 */
...
p1_dpd_outI1(struct state *p1st)
{
    ...
    time_t idle_timeout = p1st->st_connection->dpd_idle_timeout;

    dpd_outI(p1st, p1st, FALSE, delay, timeout, idle_timeout);
}
...
p2_dpd_outI1(struct state *p2st)
{
    ...
    time_t idle_timeout = p2st->st_connection->dpd_idle_timeout;
    ...
    dpd_outI(st, p2st, TRUE, delay, timeout, idle_timeout);
}
