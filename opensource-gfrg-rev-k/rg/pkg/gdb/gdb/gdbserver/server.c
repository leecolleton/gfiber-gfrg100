...
static void close_critical_err_file(void)
{
    fclose(critical_err_file);
}
...
main (int argc, char *argv[])
{
  ...
  if (!strchr (argv[1], ':')) /* Serial */
    {
      /* Serial port will be used for connection with gdb client, 
       * so we need to close any prints to stderr and stdout,
       * because they will reach serial tty device and interfere with connection
       * to gdb client. Critical errors print will be active, because critical
       * error cause gdbserver to exit.
       */
      critical_err_file = fdopen(dup(2), "w+");
      close(1);
      close(2);
      atexit(close_critical_err_file);
    }
  else
      critical_err_file = stderr;
