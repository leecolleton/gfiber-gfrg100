...
#include "utils.h"
#include "../include/ntfs/ntfstime.h"
#include "sd.h"
#include "upcase.h"
#include "boot.h"
#include "attrdef.h"
...
static s64 ntfs_rlwrite(struct ntfs_device *dev, const runlist *rl,
		const u8 *val, const s64 val_len, s64 *inited_size, int logfile)
{
		...
		const u8 *val, const s64 val_len)
{
	...
	*/
	...
	if (flags & ATTR_COMPRESSION_MASK) {
		...
		a->compression_unit = 0;
		bw = ntfs_rlwrite(g_vol->dev, rl, val, val_len, NULL, 0);
		if (bw != val_len) {
			ntfs_log_error("Error writing non-resident attribute value.\n");
			return -errno;
		}
		...
		const FILE_NAME_TYPE_FLAGS file_name_type)
{
	...
	uname = fn->file_name;
	i = ntfs_mbstoucs(file_name, &uname);
	if (i < 1) {
		free(fn);
		return -EINVAL;
	}
		...
		ntfschar* name, u32 name_len)
{
	...
	lw = ntfs_rlwrite(g_vol->dev, rl_index, (u8*)idx, i, NULL, 0);
	free(rl_index);
	if (lw != i) {
...
/* Hack to fix out of memory error on formatting large disks. Changed malloc to
 * static size instead of function of size of disk. (for an 8GB disk this can be
 * 50 Mb). As the contents of this buffer is constant (see memset) this doesn't
 * change the outcome */
#define LOGFILE_BUF_LEN 10000
	buf_log = malloc(LOGFILE_BUF_LEN);
	if (!buf_log) {
		ntfs_log_perror("Failed to allocate internal buffer");
		return FALSE;
	}
	memset(buf_log, -1, LOGFILE_BUF_LEN);
	err = add_attr_data_positioned(m, NULL, 0, 0, 0, g_rl_logfile, buf_log,
			g_logfile_size, LOGFILE_BUF_LEN);
	free(buf_log);
	buf_log = NULL;
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
	...
	err = add_attr_data_positioned(m, NULL, 0, 0, 0, g_rl_boot, (u8*)bs, 8192, 0);
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
...
static int mkntfs_redirect(struct mkntfs_options *opts2)
{
	...
	if (a->non_resident) {
		...
		lw = ntfs_rlwrite(g_vol->dev, rl, g_lcn_bitmap, g_lcn_bitmap_byte_size, NULL, 0);
		err = errno;
		free(rl);
