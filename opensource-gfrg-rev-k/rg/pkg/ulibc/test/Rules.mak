...
TARGET_ARCH:=$(shell $(CC) -DDDdumpmachine | sed -e s'/-.*//' \
	-e 's/i.86/i386/' \
	-e 's/sparc.*/sparc/' \
	-e 's/arm.*/arm/g' \
	-e 's/m68k.*/m68k/' \
...
JMK_CFLAGS         += $(XWARNINGS) $(OPTIMIZATION) $(XCOMMON_CFLAGS) $(XARCH_CFLAGS) -I$(top_builddir)include $(PTINC)
HOST_CFLAGS    += $(XWARNINGS) $(OPTIMIZATION) $(XCOMMON_CFLAGS)

JMK_LDFLAGS        := $(CPU_LDFLAGS)
ifeq ($(DODEBUG),y)
	JMK_CFLAGS        += -g
	HOST_CFLAGS   += -g
	JMK_LDFLAGS       += -g
...
JMK_LDFLAGS += -B$(top_builddir)lib -Wl,-rpath,$(top_builddir)lib -Wl,-rpath-link,$(top_builddir)lib
UCLIBC_LDSO_ABSPATH=$(shell pwd)
ifdef TEST_INSTALLED_UCLIBC
JMK_LDFLAGS += -Wl,-rpath,./
UCLIBC_LDSO_ABSPATH=/lib
endif

ifeq ($(findstring -static,$(JMK_LDFLAGS)),)
	JMK_LDFLAGS += -Wl,--dynamic-linker,$(UCLIBC_LDSO_ABSPATH)/$(UCLIBC_LDSO)
endif
...
banner := ---------------------------------
pur_showclean = echo "  "JMK_CLEAN $(notdir $(CURDIR))
