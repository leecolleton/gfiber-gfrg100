...
$(U_TARGETS): $(U_TARGET_SRCS) $(MAKE_SRCS)
	$(showlink)
	$(Q)$(CC) $(JMK_CFLAGS) $(EXTRA_CFLAGS) $(JMK_CFLAGS_$@) -c $@.c -o $@.o
	$(Q)$(CC) $(JMK_LDFLAGS) $@.o -o $@ $(EXTRA_LDFLAGS) $(JMK_LDFLAGS_$@)

$(G_TARGETS): $(U_TARGET_SRCS) $(MAKE_SRCS)
	$(showlink)
	$(Q)$(HOSTCC) $(HOST_CFLAGS) $(EXTRA_CFLAGS) $(JMK_CFLAGS_$(patsubst %_glibc,%,$@)) -c $(patsubst %_glibc,%,$@).c -o $@.o
	$(Q)$(HOSTCC) $(HOST_LDFLAGS) $@.o -o $@ $(EXTRA_LDFLAGS) $(JMK_LDFLAGS_$(patsubst %_glibc,%,$@))
...
%.so: %.c
	$(showlink)
	$(Q)$(CC) \
		$(JMK_CFLAGS) $(EXTRA_CFLAGS) $(JMK_CFLAGS_$(patsubst %_glibc,%,$@)) \
		-fPIC -shared $< -o $@ -Wl,-soname,$@ \
		$(JMK_LDFLAGS) $(EXTRA_LIBS) $(JMK_LDFLAGS_$(patsubst %_glibc,%,$@))
