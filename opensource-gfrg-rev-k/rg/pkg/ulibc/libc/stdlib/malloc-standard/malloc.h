...
#define JMALLOC_MAGIC 0xcafebabe
#define JFREE_MAGIC 0xdeadbeef
#define JMALLOC_FOOT "End"
#define JMALLOC_OVERHEAD (sizeof(jmalloc_header_t) + sizeof(JMALLOC_FOOT))

typedef struct jmalloc_header_t {
    struct jmalloc_header_t *next;
    struct jmalloc_header_t *prev;
    void *alloc_ra; /* allocator */
    void *free_ra; /* releaser */
    unsigned long len; /* requested length of memory */
    unsigned long magic;
} jmalloc_header_t;

#define MALLOC_PRINTF_MAX_LINE 255
extern char malloc_printf_buf[MALLOC_PRINTF_MAX_LINE+1];
/* We use malloc_printf instead of regular printef by two reasons:
 * 1. We can redirect output to console (if CONFIG_RG_DBG_MALLOC_2_CON is
 * defined)
 * 2. We can avoid using of malloc inside printf
 */
static inline int malloc_printf(const char *format, ...)
{
    va_list args;
    int rv, fd;
#ifdef __CONFIG_RG_DBG_MALLOC_2_CON__
    ...
    rv = vsnprintf(malloc_printf_buf, MALLOC_PRINTF_MAX_LINE, format, args);
    va_end(args);
    rv = write(fd, malloc_printf_buf, rv);
...
#ifdef __CONFIG_RG_DBG_ULIBC_MALLOC__
  jmalloc_header_t alloc_head;
#ifdef __CONFIG_RG_DBG_ULIBC_MEM_POISON__
  jmalloc_header_t quarantine;
  int              n_quarantine;
  size_t           quarantine_mem;
#endif
#endif
};
...
void   __display_chunk(mchunkptr) attribute_hidden;
