...
#if 0
	    /* Since we don't free __pthread_manager_thread_bos, valgrind 
	     * says there that the memory is definitley lost. If we don't put
	     * NULL in __pthread_manager_thread_bos valgrind is quiet.
	     * All of this has no real meaning because the process is on its
	     * way out */
