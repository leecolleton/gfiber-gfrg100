...
static uint32_t rg_get_data_idle_time(l2tp_tunnel *tunnel)
{
    void *dummy;
	...
	return 0;
    cont = ses->private;
    if (!cont || ioctl(cont->fd, L2TPGETIDLETIME, &idle_time) < 0)
	return 0;
    return idle_time;
}
...
static int rg_backend_attach(uint32_t id, char *dev_name)
{
    ...
    /* open backend and attach to it */
    if ((cont->fd = gpl_sys_rg_chrdev_open(KOS_CDT_L2TP_BACKEND, O_RDWR))
	< 0)
    {
    ...
    be_params.src = tunnel->local_addr;
    be_params.dst.s_addr = tunnel->peer_addr.sin_addr.s_addr;
    be_params.tun_out_id = tunnel->assigned_id;
    be_params.tun_in_id = tunnel->my_id;
    be_params.ses_out_id = ses->assigned_id;
    be_params.ses_in_id = ses->my_id;
    be_params.dst_port = tunnel->peer_addr.sin_port;
    be_params.src_port = htons(L2TP_PORT);
    if (ioctl(cont->fd, L2TPSETPARAMS, &be_params) < 0)
    {
	l2tp_set_errmsg("failed set backend parameters to device %s", dev_name);
	goto Error;
    }
...
int rg_init(EventSelector *es)
{
    ...
    tunnel_set_get_data_idle_time(rg_get_data_idle_time);
