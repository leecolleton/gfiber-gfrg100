...
static uint32_t (*tunnel_get_data_idle_time)(l2tp_tunnel *);

void tunnel_set_get_data_idle_time(uint32_t (*cb)(l2tp_tunnel *))
{
    tunnel_get_data_idle_time = cb;
}
		...
		void *data)
{
     ...
     * no data.
     */
    if (tunnel_get_data_idle_time)
	data_idle = tunnel_get_data_idle_time(tunnel);
    if (data_idle < 60)
	return;
