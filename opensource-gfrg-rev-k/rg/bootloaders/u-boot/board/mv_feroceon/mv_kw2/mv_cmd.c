...
#else
extern void mvEthRegs(int port);
extern void mvEthPortRegs(int port);
extern void mvNetaPortStatus(int port);
extern void mvNetaRxqShow(int port, int rxq, int mode);
extern void mvNetaTxqShow(int port, int rxq, int mode);
...
static int temp_get(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (temp < 0) {
		printf("Temperature is invalid, or unstable.\n");
		return 0;
	}
