...
static int mv_eth_netconfig_ports_get(char **p_net_config, int idx, int port)
{
	...
	*p_net_config = config_str;
	...
	printk("Syntax error while parsing switch port mask from command line\n");
	return -EINVAL;
}
...
int mv_eth_switch_config_get(int use_existing_config, int port)
{
	...
	if (!use_existing_config) {
		...
		if (net_config_str[port] != NULL) {
			printk("      o Using UBoot netconfig string for port %d\n", port);
		}
		else {
			printk("      o Using default netconfig string from Kconfig for port %d\n", port);
		...
		for (i = mv_eth_switch_netdev_first; i <= mv_eth_switch_netdev_last; i++) {		
			if (mv_net_devs[i] != NULL)
				memcpy(switch_net_config[port].mac_addr[i - mv_eth_switch_netdev_first], mv_net_devs[i]->dev_addr, MV_MAC_ADDR_SIZE);
		}
...
int     mv_eth_switch_change_mtu(struct net_device *dev, int mtu)
{
	...
	for (i = mv_eth_switch_netdev_first; i <= mv_eth_switch_netdev_last; i++) {
		if ((mv_net_devs[i] != NULL) && (netif_running(mv_net_devs[i]))) {
			printk(KERN_ERR "All gateway interfaces must have be down before changing MTU. %s is not down\n",
					mv_net_devs[i]->name);
			return -EPERM;
		}
	}
