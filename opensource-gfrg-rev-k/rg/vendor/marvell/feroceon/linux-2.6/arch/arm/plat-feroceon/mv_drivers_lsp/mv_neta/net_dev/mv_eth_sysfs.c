...
static ssize_t mv_eth_help(char *buf)
{
	...
	off += sprintf(buf+off, "echo {0|1}         > pnc_cfg       - enable / disable PNC configuration by Linux net device\n");
				   ...
				   struct device_attribute *attr, const char *buf, size_t len)
{
	...
	else if (!strcmp(name, "pnc_cfg")) {
		mv_eth_config_parser_set(p);
	}
...
static DEVICE_ATTR(pnc_cfg,	    S_IWUSR, mv_eth_show, mv_eth_port_store);
