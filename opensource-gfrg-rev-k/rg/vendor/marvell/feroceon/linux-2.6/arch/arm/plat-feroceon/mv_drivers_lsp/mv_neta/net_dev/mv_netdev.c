...
#else
unsigned int config_pnc_parser = 0;
#endif
...
static void mv_eth_add_tx_done_timer(struct eth_port *pp)
{
	if (!(pp->flags & MV_ETH_F_TX_DONE_TIMER)) {
		...
		/* Two execution paths can reach this point "at the same time":
		 * interupt handler (called by mv_eth_tx_done_timer_callback()) and from
		 * mv_eth_tx(). So using mod_timer instead of add_timer. */
		mod_timer(&pp->tx_done_timer, jiffies + ((HZ * CONFIG_MV_ETH_TX_DONE_TIMER_PERIOD) / 1000));
	}
}
...
int mv_eth_ctrl_pool_size_set(int pool, int pkt_size)
{
	...
	for (port = 0; port < mv_eth_ports_num; port++) {
		...
		if (ppool->port_map & (1 << port)) {
			...
			if (NULL == pp){
				printk(KERN_ERR "Skipping port %d, not initialized\n", port);
				continue;
			}
...
int mv_eth_ctrl_rxq_size_set(int port, int rxq, int value)
{
	...
	if (NULL == pp){
		printk(KERN_ERR "Port %d, not initialized\n", port);
		return -ENODEV;
	}
								...
								struct mv_eth_tx_spec *tx_spec_out))
{
    ...
    if (pp == NULL)
    {
        printk("mv_eth_rx_special_proc_func - NULL pointer\n");
        return;
    }
...
MV_U32 aei_gpio_led_grp, aei_gpio_led_mask, aei_gpio_led_val;
...
void mv_eth_skb_print(struct sk_buff *skb)
{
	...
	printk("\t users=%d dataref=%d nr_frags=%d gso_size=%d gso_segs=%d\n",
...
static inline int mv_eth_tx_policy(struct eth_port *pp, struct sk_buff *skb)
{
	...
	if (skb->protocol == htons(ETH_P_IP) && (skb->nh.iph))
#else
...
static inline int mv_eth_tso_validate(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (skb->len <= skb_shinfo(skb)->gso_size) {
		printk("***** ERROR: total_len (%d) less than gso_size (%d)\n", skb->len, skb_shinfo(skb)->gso_size);
		return 1;
	}
...
/*
		printk("%s: no TX descriptors - txq_count=%d, len=%d, nr_frags=%d, gso_segs=%d\n",
...
*/
	...
	while (total_len > 0) {
		...
		size = mv_eth_tso_build_hdr_desc(tx_desc, priv, skb, txq_ctrl, hdr_len, data_left, tcp_seq, ip_id, total_len);
		totalBytes += size;
/*
...
#ifdef CONFIG_MV_PON
	if (MV_PON_PORT(pp->port))
		mvNetaPonTxqBytesAdd(priv->port, tx_spec->txp, tx_spec->txq, totalBytes);
#endif /* CONFIG_MV_PON */
...
outNoTxDesc:
	...
	printk("%s: No TX descriptors - rollback %d, txq_count=%d, nr_frags=%d, skb=%p, len=%d, gso_segs=%d\n",
...
#endif
{
	...
	pp->causeRxTx[smp_processor_id()] = 0;
	...
	for (port = 0; port < mv_eth_ports_num; port++) {
		...
		if (pp) {
			if (!(pp->flags & MV_ETH_F_CONNECT_LINUX)) {
				if (pp->flags & MV_ETH_F_SWITCH)
					printk("%s: a GbE port using the Gateway driver cannot be disconnected from Linux\n", __func__);
				else
					mv_eth_open(pp->dev);
			}
		}
...
#   error "If GSO enabled - TX checksum offload must be enabled too"
	...
	printk("     o Loop detection support\n");
...
int mv_eth_txp_reset(int port, int txp)
{
	...
	if (NULL == pp) {
		printk(KERN_ERR "%s - pp is NULL for port (%d)", __FUNCTION__, port);
		return -ENODEV;
	}
...
int mv_eth_start_internals(struct eth_port *pp, int mtu)
{
	...
	for (txp = 0; txp < pp->txp_num; txp++) {
		for (txq = 0; txq < CONFIG_MV_ETH_TXQ; txq++) {
			...
			mvNetaTxDonePktsCoalSet(pp->port, txp, txq, txq_ctrl->txq_done_pkts_coal);
		}
...
int mv_eth_change_mtu_internals(struct net_device *dev, int mtu)
{
	...
	if ((mtu != dev->mtu) && (pp->pool_long)) {
		...
		if ((new_pool == NULL) || (new_pool->pkt_size < RX_PKT_SIZE(mtu)) || (pp->pool_long != new_pool) ||
			(mv_eth_bm_config_pkt_size_get(new_pool->pool) == 0)) {
			mv_eth_rx_reset(pp->port);
...
void mv_eth_pool_status_print(int pool)
{
	...
	printk("skb_recycled_ok=%u, skb_recycled_err=%u\n",
...
int mv_eth_config_parser_set (unsigned int pnc_parser_value)
{
#ifndef CONFIG_MV_ETH_PNC_PARSER
    if (pnc_parser_value) {
		printk("%s: Not allowed, kernel was not compiled with CONFIG_MV_ETH_PNC_PARSER.\n", __func__);
        return 1;
    }
#endif
    config_pnc_parser = pnc_parser_value;
    return 0;
}

int mv_eth_config_parser_get (unsigned int *ret_pnc_parser_value)
{
    *ret_pnc_parser_value = config_pnc_parser;
    return 0;
}
