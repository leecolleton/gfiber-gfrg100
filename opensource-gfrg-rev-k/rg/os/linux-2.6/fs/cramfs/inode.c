 ...
 * CRAMFS_SUB_TYPE_DLMFS - cramfs_dlmfs with CONFIG_RG_DLM files,
 *   embedded to kernel and mounted from it (RAM), memory is freed on umount
 */
#define CRAMFS_SUB_TYPE_DEV 0
#define CRAMFS_SUB_TYPE_MAINFS 1
#define CRAMFS_SUB_TYPE_MODFS 2
#define CRAMFS_SUB_TYPE_DLMFS 3
...
#ifdef CONFIG_RG_DLM
extern char __dlmfs_start, __dlmfs_end;
#endif
...
#ifdef CONFIG_RG_DLM
    else if (CRAMFS_SB(sb)->sub_type == CRAMFS_SUB_TYPE_DLMFS)
	return &__dlmfs_start;
#endif
	...
	unsigned int len)
{
	...
	return cramfs_dev_read(sb, offset, len);
...
#ifdef CONFIG_RG_DLM
static int cramfs_dlmfs_fill_super(struct super_block *sb,
	void *data, int silent)
{
	return cramfs_fill_super(sb, data, silent, CRAMFS_SUB_TYPE_DLMFS);
}
#endif
...
static int cramfs_get_mainfs_sb(struct file_system_type *fs_type,
...
static int cramfs_get_modfs_sb(struct file_system_type *fs_type,
	int flags, const char *dev_name, void *data, struct vfsmount *mnt)
{
	return get_sb_nodev(fs_type, flags, data, cramfs_modfs_fill_super, mnt);
}

#ifdef CONFIG_RG_DLM
static int cramfs_get_dlmfs_sb(struct file_system_type *fs_type,
	int flags, const char *dev_name, void *data, struct vfsmount *mnt)
{
	return get_sb_nodev(fs_type, flags, data, cramfs_dlmfs_fill_super, mnt);
}
#endif
...
#ifdef CONFIG_RG_DLM
static void cramfs_kill_dlmfs_sb(struct super_block *sb)
{
	unsigned long start = (unsigned long)&__dlmfs_start;
	unsigned long end = (unsigned long)&__dlmfs_end;
...
#endif
	free_area(start, end, "CRAMFS/DLMFS");
}
#endif
...
#ifdef CONFIG_RG_DLM
static struct file_system_type cramfs_dlmfs_type = {
	.owner		= THIS_MODULE,
	.name		= "cramfs_dlmfs",
	.get_sb		= cramfs_get_dlmfs_sb,
	.kill_sb	= cramfs_kill_dlmfs_sb,
};
#endif
...
#ifdef CONFIG_RG_DLM
	register_filesystem(&cramfs_dlmfs_type) ||
#endif
...
#ifdef CONFIG_RG_DLM
    unregister_filesystem(&cramfs_dlmfs_type);
#endif
}
