...
unsigned int cramfs_uncompress_counter=0;
...
void cramfs_uncompress_exit_gzip(void)
{
	if (!--gzip_initialized) {
		zlib_inflateEnd(&stream);
...
void cramfs_uncompress_exit_lzma(void)
{
	if (!--lzma_initialized)
		lzma_decode_uninit();
}
...
void cramfs_uncompress_exit(int comp_method)
{
     	switch (comp_method)
	{
	case CRAMFS_FLAG_COMP_METHOD_GZIP:
		cramfs_uncompress_exit_gzip();
		break;
	case CRAMFS_FLAG_COMP_METHOD_LZMA:
		cramfs_uncompress_exit_lzma();
		break;
	}
}
