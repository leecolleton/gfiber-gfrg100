				    ...
				    const char * key, char * signature)
{
	...
	_MD5Init(&context);
	_MD5Update(&context,key,CIFS_SESS_KEY_SIZE+16);
	_MD5Update(&context,cifs_pdu->Protocol,cifs_pdu->smb_buf_length);
	_MD5Final(signature,&context);
	return 0;
}
				...
				const char * key, char * signature)
{
	...
	for(i=0;i<n_vec;i++) {
		...
		if(i==0) {
			...
			_MD5Update(&context,iov[0].iov_base+4, iov[0].iov_len-4);
		} else
			_MD5Update(&context,iov[i].iov_base, iov[i].iov_len);
	}
