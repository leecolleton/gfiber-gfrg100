...
request_standard_resources(struct meminfo *mi, struct machine_desc *mdesc)
{
	...
	kernel_data.end     = virt_to_phys(&_end + ACTUAL_END_OFFSET - 1);
...
extern unsigned long rofs_start_addr, rofs_len, is_in_flash;
...
void __init setup_arch(char **cmdline_p)
{
...
#ifdef CONFIG_RG_MAINFS_CRAMFS
	rofs_start_addr = virt_to_phys(&__mainfs_start);
	rofs_len = &__mainfs_end - &__mainfs_start;
	...
	init_mm.brk	   = (unsigned long) &_end + ACTUAL_END_OFFSET;

	cmdline_len = strlen(default_command_line);
#ifdef CONFIG_MTD
	from += cmdline_len;
	openrg_commandline_parse(from, rofs_start_addr, rofs_len,
	    cramfs_location, cCONFIG_MTD_PHYSMAP_START, 
	...
	if (!*from)
	{
	    printk("No arch command line. Setting to default");
	    memcpy(default_command_line, CONFIG_CMDLINE, COMMAND_LINE_SIZE);
