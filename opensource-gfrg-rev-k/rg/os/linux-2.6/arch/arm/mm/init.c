...
extern unsigned long rofs_start_addr, rofs_len;
extern cramfs_location_t cramfs_location;
...
find_bootmap_pfn(int node, struct meminfo *mi, unsigned int bootmap_pages)
{
	...
	start_pfn   = PAGE_ALIGN(__pa(((unsigned long)&_end) +
	    ACTUAL_END_OFFSET)) >> PAGE_SHIFT;
	bootmap_pfn = 0;
...
static int __init check_node(struct meminfo *mi, unsigned long phys_start,
    unsigned long phys_size)
{
	int node = -2;
	unsigned long end = phys_start + phys_size;

	/*
	 * Make sure that addresses are within a valid area of
	 * memory.
	 */
	if (phys_size) {
		unsigned int i;
...
static int __init check_initrd(struct meminfo *mi)
{
    ...
    node = check_node(mi, phys_initrd_start, phys_initrd_size);
...
#ifdef CONFIG_RG_KLOG_RAM_BE_FEATURE
unsigned long klog_ram_paddr = 0;
EXPORT_SYMBOL(klog_ram_paddr);
#endif
...
#ifdef CONFIG_RG_KLOG_RAM_BE_FEATURE
#ifndef CONFIG_RG_KLOG_RAM_LOCATION
	klog_ram_paddr = __pa(high_memory - CONFIG_RG_KLOG_RAMSIZE);
#else
	klog_ram_paddr = CONFIG_RG_KLOG_RAM_LOCATION;
#endif
	klog_node = check_node(mi, klog_ram_paddr, CONFIG_RG_KLOG_RAMSIZE);
	if (klog_node >= 0)
	{
	    reserve_bootmem_node(NODE_DATA(klog_node), klog_ram_paddr,
		CONFIG_RG_KLOG_RAMSIZE);
	    printk("Reserved %dk KLOG memory starting from Physical 0x%p\n", 
		(int)(CONFIG_RG_KLOG_RAMSIZE/1024), 
		(void*)(klog_ram_paddr));
	}
	else
	    klog_ram_paddr = 0;
...
void __init mem_init(void)
{
	...
	datapages = (&_end - &__data_start) + ACTUAL_END_OFFSET;
