 ...
 *
 * Version:	$Id: af_packet.c,v 1.9 2012/02/23 12:17:38 moran Exp $
 *
 * Authors:	Ross Biro
...
static int _packet_create(struct socket *sock, int protocol, int pt_flags)
{
	struct sock *sk;
	struct packet_sock *po;
...
{
	return _packet_create(sock, protocol, 0);
}

#ifdef CONFIG_RG_PCAP_SOCKET
static int pcap_create(struct socket *sock, int protocol)
{
	return _packet_create(sock, protocol, PTYPE_PRE_HOOK);
}
#endif
...
#ifdef CONFIG_RG_PCAP_SOCKET
static struct net_proto_family pcap_family_ops = {
	.family =       PF_PCAP,
	.create =       pcap_create,
	.owner	=	THIS_MODULE,
};
