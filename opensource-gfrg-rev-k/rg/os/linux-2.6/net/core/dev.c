...
int (*net_rx_hook)(struct sk_buff **skb) = net_hook_stub;
int (*net_tx_hook)(struct sk_buff **skb) = net_hook_stub;

EXPORT_SYMBOL(net_rx_hook);
EXPORT_SYMBOL(net_tx_hook);
...
int (*net_local_hook)(struct sk_buff **skb, int is_local_out, 
        u16 pfamily) = net_local_hook_stub;
...
void dev_set_local_hook(int (*hook)(struct sk_buff **skb, int is_local_out, u16 pf_family))
{
...
static int netdev_pre_hook_packs;
...
void dev_add_pack(struct packet_type *pt)
{
	...
	if (pt->flags & PTYPE_PRE_HOOK)
		netdev_pre_hook_packs++;
...
static void net_rx_pre_hook_packs(struct sk_buff *skb, 
    struct net_device *orig_dev) 
{
	struct packet_type *ptype;
	__be16 type;
...
int netif_receive_skb(struct sk_buff *skb)
{
	...
	if (netdev_pre_hook_packs > 0)
		net_rx_pre_hook_packs(skb, orig_dev);
