...
#ifdef CONFIG_CENTILLIUM_P400
#include <asm/palladia/iad_chiron_defs.h>
#endif
...
{
	return readb(kgdb8250_addr + (mask << current_port->regshift));
}

static inline void kgdb_iowrite(u8 val, u8 mask)
{
	writeb(val, kgdb8250_addr + (mask << current_port->regshift));
}
...
#elif CONFIG_CENTILLIUM_P400
	base_baud = IAD_UART_CLOCK_RATE_C2 / 16;
#endif	
	/* set DLAB */
