...
serial_out(struct uart_8250_port *up, int offset, int value)
{
	...
	case UPIO_DWAPB:
		 ...
		 ** we need to make sure that the busy bit is cleared in
		 * USR register.
		 */
		if ((((readb(up->port.membase + 0xC) & 0x80) &&
			((save_offset == UART_DLL) ||
			(save_offset == UART_DLM) ||
			(offset == 0x1C))) || (save_offset == UART_LCR)) &&
			!(readb(up->port.membase + 0x14) & 0x1)) {
			unsigned int status;
			do {
				status = *((volatile u32 *)
