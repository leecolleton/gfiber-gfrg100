...
/**
 * ubi_attach_mtd_dev_nm - attach an MTD device by name.
 * @mtd_name: MTD device name
 * @ubi_num: number to assign to the new UBI device
 * @vid_hdr_offset: VID header offset
 *
 * This function obtains an MTD device by @mtd_name, attaches it to UBI and 
 * assigns @ubi_num number to the newly created UBI device, unless @ubi_num is 
 * %UBI_DEV_NUM_AUTO, in which case this function finds a vacant device number 
 * and assigns it automatically. 
 * Returns the new UBI device number in case of success and a negative error 
 * code in case of failure.
 *
 * Notes:
 *  - The invocations of this function are serialized.
 *  - The MTD device obtained by @mtd_name is held
 */
int ubi_attach_mtd_dev_nm(char *mtd_name, int ubi_num, 
			  int vid_hdr_offset)
{
	int err;
	struct mtd_info *mtd;

	if (ubi_num < 0 && ubi_num != UBI_DEV_NUM_AUTO)
		return -EINVAL;
	...
	mutex_lock(&ubi_devices_mutex);
	err = ubi_attach_mtd_dev(mtd, ubi_num, vid_hdr_offset);
	mutex_unlock(&ubi_devices_mutex);
	if (err < 0)
		put_mtd_device(mtd);
	return err;
}
EXPORT_SYMBOL_GPL(ubi_attach_mtd_dev_nm);
