...
_sock_recv_timestamp(struct msghdr *msg, struct sock *sk, struct sk_buff *skb)
{
...
static __inline__ void
sock_recv_mark(struct msghdr *msg, struct sock *sk, struct sk_buff *skb)
{
	if (sock_flag(sk, SOCK_RCVMARK)) {
	    put_cmsg(msg, SOL_PACKET, PACKET_MARK, sizeof(__u32),
		&skb->mark);
	}
}

static __inline__ void
sock_recv_packet_info(struct msghdr *msg, struct sock *sk, struct
    sk_buff *skb)
{
    _sock_recv_timestamp(msg, sk, skb);
